<template>
  <el-card shadow="never" class="border-1">
    ...
    <el-table :data="tableData" height="680px">
      ...
      <el-table-column prop="defectName"  label="缺陷种类名称"  align="center"/>
      ...
      <el-table-column label="操作"  align="center">
        <template #default="scope">
          ...
          <el-popconfirm title="您确定要删除吗?" confirm-button-text="确定" cancel-button-text="取消" @confirm="handleDelete(scope.row.defectID)" :disabled="!isAdmin">
            <template #reference>
    ...
    <FormDrawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit">
     <el-form :model="form" ref="formRef" :rules="rules" label-width="120px" :inline="false">
       <el-form-item label="缺陷种类名称" prop="defectName">
         <el-input v-model="form.defectName" placeholder="缺陷种类名称" />
       </el-form-item>
       <el-form-item label="缺陷等级" prop="defectGrade">
         <el-input v-model="form.defectGrade" placeholder="缺陷等级" />
       </el-form-item>
     </el-form>
    </FormDrawer>
  </el-card>
...
getmDefectList(currentPage.value, pageSize.value)
  .then(res => {
...
//修改标识 0：新增 其他：修改
const editId = ref(0)
const drawerTitle = computed(()=> editId.value === 0 ? '新增缺陷种类' : '修改缺陷种类')
const handleSubmit = ()=>{
formRef.value.validate((valid)=>{
  if(!valid) return

  const fun = editId.value ? updateDefect(form) : addDefect(form)
